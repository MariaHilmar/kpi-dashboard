-- Corrige pivô por épico: universo de linhas deve usar issues.epico (mesma chave do agg),
-- não o catálogo gitlab_epics cujos títulos podem divergir de issues.epico.
...
as $$
  ...
  universe_epicos_todos as (
    select distinct coalesce(nullif(trim(i.epico), ''), 'Não informado') as linha
    from public.issues i
    where coalesce(i.ano_criacao, 0) >= 2024
  ),
  universe_epicos_modulo as (
    ...
    where coalesce(i.ano_criacao, 0) >= 2024
      and (
        i.modulo = p_modulo
        or coalesce(nullif(trim(i.epico), ''), '') = ''
