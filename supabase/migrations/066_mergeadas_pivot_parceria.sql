-- Pivô de mergeadas: adiciona 'parceria' como dimensão de linhas (além de módulo/épico).
-- Mantém compatibilidade: p_linha_dimensao 'modulo' -> módulo; 'parceria' -> parceria;
-- qualquer outro valor (ex.: 'epico') -> épico (comportamento anterior de "não-módulo").
...
as $$
  with dim as (
    select lower(trim(coalesce(p_linha_dimensao, 'modulo'))) as v
  ),
  f_base as (
  ...
  agg_parceria as (
    select
      coalesce(nullif(trim(f.parceria), ''), 'Não informado') as linha,
      to_char(date_trunc('month', f.mergeado_em), 'YYYY/MM') as periodo,
      count(*)::bigint as total
    from f
    where f.mergeado_em is not null
  ...
  agg as (
    select * from agg_modulo where (select v from dim) = 'modulo'
    union all
    select * from agg_parceria where (select v from dim) = 'parceria'
    union all
    select * from agg_epico where (select v from dim) not in ('modulo', 'parceria')
  ),
  universe as (
    select linha from universe_modulos where (select v from dim) = 'modulo'
    union
    select distinct a.linha from agg_parceria a where (select v from dim) = 'parceria'
    union
    select distinct a.linha from agg_epico a where (select v from dim) not in ('modulo', 'parceria')
  )
  select
    u.linha,
    p.periodo,
