-- Resolve épico do pivô via issues.epico OU vínculo GitLab (gitlab_epic_issue_links).
-- Corrige contagem toda em "Não informado" quando o catálogo tem títulos mas
-- issues.mergeadas ainda não tinham issues.epico preenchido no sync.

create table if not exists public.gitlab_epic_issue_links (
  id uuid primary key default gen_random_uuid(),
  gitlab_group_path text not null default 'comprasnet',
  gitlab_repo text not null,
  gitlab_iid integer not null,
  gitlab_epic_id bigint,
  epic_title text not null,
  synced_at timestamptz not null default now(),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint gitlab_epic_issue_links_repo_iid_unique unique (gitlab_repo, gitlab_iid)
);

create index if not exists idx_gitlab_epic_issue_links_repo_iid
  on public.gitlab_epic_issue_links (gitlab_repo, gitlab_iid);

create index if not exists idx_gitlab_epic_issue_links_title
  on public.gitlab_epic_issue_links (epic_title);

grant select on public.gitlab_epic_issue_links to anon, authenticated, service_role;
grant insert, update, delete on public.gitlab_epic_issue_links to service_role;
  ...
  agg_epico as (
    ...
    where f.mergeado_em is not null
      and (
        ...
        or coalesce(nullif(trim(f.epico_resolvido), ''), 'Não informado') = 'Não informado'
        or substring(trim(f.epico_resolvido) from '^\s*\[([^\]]+)\]') = p_modulo
      )
    group by 1, 2
  ),
  universe_modulos as (
