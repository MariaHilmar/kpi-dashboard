-- Pivô por épico: linhas = só épicos que aparecem no agg (sem catálogo inteiro com zero).
-- O catálogo gitlab_epics gerava dezenas de linhas vazias enquanto issues.epico estava vazio.
...
as $$
  ...
  universe as (
    ...
    where lower(trim(coalesce(p_linha_dimensao, 'modulo'))) = 'modulo'
    union
    select distinct a.linha from agg_epico a
    where lower(trim(coalesce(p_linha_dimensao, 'modulo'))) <> 'modulo'
  )
  select
    u.linha,
